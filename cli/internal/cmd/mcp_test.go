...
func TestMcpEndpoint_TextForm_DerivesFromAPIURL(t *testing.T) {
	...
	// docs/public/mcp.md is explicit: the Streamable HTTP transport serves at
...
func TestMcpEndpoint_JSON(t *testing.T) {
	...
	if v.URL != "https://mcp.hail.so" {
		t.Fatalf("url mismatch: %q (canonical per docs/public/mcp.md is root path, no /mcp)", v.URL)
	}
