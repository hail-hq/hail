...
export function mapCsvToNumbers(csvText) {
  ...
  // Every skipped row is recorded with its reason: a silent drop here removes
  // a (country, type) from the acquire allow-list, which can strand a held
  // number unbilled — the caller must surface these.
  const dropped = [];
  for (const r of parseCsv(csvText)) {
    ...
    if (!voice && !sms) {
      dropped.push({
        iso: r["ISO"],
        type: r["Phone Number Type"],
        reason: "no voice or sms capability",
      });
      continue;
    }
    const priceRaw = r["Phone Number Price / month"];
    if (!/^(0|[1-9][0-9]*)(\.[0-9]{1,8})?$/.test(priceRaw)) {
      dropped.push({
        iso: r["ISO"],
        type: r["Phone Number Type"],
        reason: `unparseable price ${JSON.stringify(priceRaw)}`,
      });
      continue;
    }
    const iso = r["ISO"];
...
async function main() {
  ...
  if (!res.ok)
    throw new Error(`Twilio CSV fetch failed: ${res.status} (${CSV_URL})`);
  const csv = await res.text();
  const { rows, countryCount, dropped } = mapCsvToNumbers(csv);
  for (const d of dropped) {
    console.warn(`dropped row ${d.iso}/${d.type}: ${d.reason}`);
  }
  if (countryCount < COUNTRY_FLOOR) {
    throw new Error(
      `sync aborted: only ${countryCount} countries (< ${COUNTRY_FLOOR}); refusing to shrink the allow-list`,
    );
  }
  const existing = JSON.parse(await readFile(TELEPHONY_JSON, "utf-8"));
  // A (country, type) that vanishes from the feed strands any held number of
  // that type unbilled; the country floor can't catch a single-row loss, so
  // call out removals explicitly for the PR reviewer.
  ...
  for (const k of removed) {
    console.warn(
      `REMOVED from allow-list: ${k} — verify no held numbers of this type before merging`,
    );
  }
