...
`dns-check <id>` is read-only: it detects the tenant's DNS host and checks whether DKIM/MX/SPF and DMARC are live in public DNS. It never changes `verification_status` (run `verify` for that). `kind='hail_mail'` rows do no lookups, so the host line reads `not recognised`, and there's no table or DMARC line — that zone is Hail's, not the customer's. Otherwise, output is a `DNS host: <name> — <url>` line, a TYPE/NAME/VALUE/SEEN table (`SEEN` = seen in public DNS, not verified), and a `DMARC: found` or `DMARC: not found — add TXT <name> "<value>"` line. If a lookup fails or times out, it prints one line — `DNS check could not be completed. Try again in a minute.` — instead of guessing at missing records.
