...
4. **[Gmail accounts](./gmail-accounts.md)** — connect a real Gmail mailbox to send-as-yourself and read inbox live. Independent of SES; optional.
5. **[MCP clients](./mcp.md)** — connect Claude.ai / ChatGPT / Cursor / Claude Code to your Hail deployment. Independent of voice setup; needed only for agent-driven flows.
6. **[VM deployment](./vm-deploy.md)** — run the whole stack on a single Ubuntu VM behind HTTPS, with GitHub Actions auto-deploying each commit on `main`.
