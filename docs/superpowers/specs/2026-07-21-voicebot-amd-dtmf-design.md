...
| `machine-ivr`         | Suppress the greeting; take an LLM turn (`generate_reply`) carrying the captured menu text so the model can press keys via `send_dtmf`. Disclosure deferred to the first person who speaks. **Corrected 2026-07-21** — this originally said "same as `human`, the disclosure goes into the phone tree, which is harmless". It was not harmless: `session.say` is TTS-only and never invokes the LLM, so on call `fe1d2d08` the agent recited its disclosure at a "press one / press two" menu and pressed nothing. |
