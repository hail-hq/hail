...
- **`idempotency_keys` lifecycle (GC + in-flight reaper)** — gate on this before v1 GA. Task 7 ships the table and the INSERT-ON-CONFLICT acquire path but no cleanup. Two pieces still owe: (1) a periodic `DELETE FROM idempotency_keys WHERE expires_at < now()` (cron / startup task) — without it the table grows unboundedly and the PK index slows the acquire path; (2) a reaper for rows stuck at the in-flight sentinel (`response_status = 0`) because the handler crashed between acquire and store — without it those keys 409 "still processing" for the full 24h TTL. Both are operational, not behavioral.
