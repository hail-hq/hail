...
locals {
  ...
  ses_tracking_enabled  = run_cmd("--terragrunt-quiet", "bash", "-c", "source ${local.env_file} && echo -n \"$HAIL_SES_TRACKING_ENABLED\"")
}
...
inputs = {
  ...
  ses_tracking_domain_enabled = local.ses_tracking_enabled == "true"
